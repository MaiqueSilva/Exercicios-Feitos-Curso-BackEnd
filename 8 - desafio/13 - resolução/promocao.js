...
let valorParaPagamento = 0

if (precos.length <= 2) {
    valorParaPagamento = precos[0] + precos[1];
    console.log(valorParaPagamento);
} else {
    let desconto = Math.min(...precos) * 50 / 100;

    for (let item of precos) {
        valorParaPagamento += item;
    }
    let valorFInal = valorParaPagamento - desconto;
