...
select nome, idade, cast(cadastro as date) from usuarios where cast(cadastro as date) between cast('2019-12-31' as date) and cast('2021-01-01' as date);
...
select categoria, count(estoque) as "quantidade" from farmacia where categoria IS NOT NULL group by categoria
...
select idade, count(cadastro) as "quantidade" from usuarios group by idade order by idade asc;
